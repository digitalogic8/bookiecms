...
def buildGames(sport="NCAAF"):
    from better.models import Team, Contest, AvailableBets
    data = scrapeOdds(sport)
    for gameData in data["games"]:
        try:
            awayTeam = Team.objects.filter(teamAbbreviation=gameData["away"], sport=sport)[0]
        except:
            print (gameData["away"] + " is not in the db")
            continue
        try:
            homeTeam = Team.objects.filter(teamAbbreviation=gameData["home"], sport=sport)[0]
        except:
...
def buildTeams():
    ...
    with open('/code/util/ncaaf/ncaafTeams.csv', 'r') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
        for row in spamreader:
            if len(Team.objects.filter(teamAbbreviation=row[0], sport="NCAAF")) == 0:
                team = Team(teamAbbreviation=row[1], teamName=row[0], sport="NCAAF", teamLogo=row[5])
                team.save()
