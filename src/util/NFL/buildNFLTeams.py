...
def buildTeams():
    ...
    with open('/code/util/NFL/nflTeams.csv', 'r') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
        for row in spamreader:
            if len(Team.objects.filter(teamAbbreviation=row[0], sport="NFL")) == 0:
                team = Team(teamAbbreviation=row[2], teamName=row[1], sport="NFL", teamLogo=row[5])
                team.save()
